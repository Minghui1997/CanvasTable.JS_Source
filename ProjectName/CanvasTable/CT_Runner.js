...
{
    if(CT_ContainerTask.length !== 0) //如果子程序容器长度不为0
    {
        var i;
        for(i=0;i<CT_ContainerTask.length;i++) //遍历子程序容器
        {
            if(CT_ContainerTask[i] != null) //如果当前子程序不为空
            {
                if(CT_TaskStateList[i] != null) //如果当前子程序状态不为空
                {
                    if(CT_TaskStateList[i] === 0) //如果当前子程序状态为等待
                    {
                        CT_TaskRunIndex = i; //子程序运行索引
                        CT_ContainerTask[i][0](); //执行子程序main函数
                        CT_TaskStateList[i] = 1; //将当前子程序状态设置为运行
                    }
                }
            }
        }
    ...
    if(CT_ContainerTask.length !== 0) //如果子程序容器长度不为0
    {
        ...
        for(i=0;i<CT_ContainerTask.length;i++) //遍历子程序容器
        {
            if(CT_ContainerTask[i] != null) //如果当前子程序不为空
            {
                if(CT_TaskStateList[i] != null) //如果当前子程序状态不为空
                {
                    if(CT_TaskStateList[i] === 1) //如果当前子程序状态为运行
                    {
                        CT_TaskRunIndex = i; //子程序运行索引
