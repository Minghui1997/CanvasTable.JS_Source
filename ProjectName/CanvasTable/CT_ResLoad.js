...
function CT_ResourceLoad()
{
    ...
    {
        var res_name = CT_ResourceList[CT_ResLength][0].slice(CT_ResourceList[CT_ResLength][0].lastIndexOf("/")+1); //获取资源文件名
        if(res_name.lastIndexOf(".") !== -1) //如果资源文件名中有"."
        {
            if(res_name.lastIndexOf(".") !== 0) //如果资源文件名中"."的位置不为0
            {
                ...
                if(res_extname === "png" || res_extname === "jpg" || res_extname === "jpeg" || res_extname === "bmp" || res_extname === "mp3" || res_extname === "ogg") //如果资源文件类型是图像和音频
                {
                    ...
                    {
                        CT_ResourceLoad_Image(CT_ResourceList[CT_ResLength][0]);
                    }
                    if(res_extname === "mp3" || res_extname === "ogg") //如果资源文件是音频
                    {
                        CT_ResourceLoad_Audio(CT_ResourceList[CT_ResLength][0]);
                    }
                }
                else //如果资源文件类型不是图像和音频
                {
                    CT_ResourceLoad_Text(CT_ResourceList[CT_ResLength][0]);
                }
            }
            else
            {
                cprint("ERROR : The resource file name is error : " + CT_ResourceList[CT_ResLength][0]);
            }
        }
        else
        {
            cprint("ERROR : The resource file name is not support : " + CT_ResourceList[CT_ResLength][0]);
        }
    }
    else //如果资源列表长度为0
    {
...
function CT_ResourceLoad_Image(path)
{
    ...
    image.onload = function() //如果图像加载完成
    {
        ...
        //将图像参数存入资源列表
        CT_ResourceList[CT_ResLength][1] = 0;
        CT_ResourceList[CT_ResLength][2] = image_w;
        CT_ResourceList[CT_ResLength][3] = image_h;
        cprint("Resource load : " + path);
        CT_loadBarWidget(); //加载条循环
        CT_ResLength ++; //资源长度 +1
        if(CT_ResLength === CT_ResourceList.length) //如果资源加载完成
        {
            CT_ResourceLoadEnd(); //结束加载资源
        }
        else //如果资源没有加载完
...
function CT_ResourceLoad_Audio(path)
{
    ...
    CT_ResourceList[CT_ResLength][1] = 1; //将音频参数存入资源列表
    cprint("Resource load : " + path);
    CT_loadBarWidget(); //加载条循环
    CT_ResLength ++; //资源长度 +1
    if(CT_ResLength === CT_ResourceList.length) //如果资源加载完成
    ...
    xmlhttp.onreadystatechange = function() //ajax状态
    {
        if(xmlhttp.readyState === 4)
        {
            if(xmlhttp.status === 200) //如果成功读取文本文件
            {
                ...
                //将文本参数存入资源列表
                CT_ResourceList[CT_ResLength][1] = 2;
                cprint("Resource load : " + CT_ResourceList[CT_ResLength][0]);
                CT_loadBarWidget(); //加载条循环
                CT_ResLength ++; //资源长度 +1
                if(CT_ResLength === CT_ResourceList.length) //如果资源加载完成
                {
        ...
        {
            CT_LoadBarShowError(0,CT_ResourceList[CT_ResLength][0]); //显示错误信息
        }
    };
}
//--------------------------------------------------------------------
...
function CT_LoadBarShow(n)
{
    ...
    if(CT_LOADBAR_SHOW === 1) //如果加载条显示
    {
        ...
        CTBCanvas_Rectangle(bar_x,bar_y,400,30,bar_c,0); //绘制加载条
        CTBCanvas_Rectangle(bar_x+1,bar_y+1,398,28,bar_c,0); //绘制加载条
        if(n >= 1 && n <= 14) //如果加载条块索引在指定范围内
        {
            var lump_x = bar_x + 4 + (n - 1) * lump_w; //加载条块位置x
            var lump_y = bar_y + 4; //加载条块位置y
            CTBCanvas_Rectangle(lump_x,lump_y,lump_w,lump_h,bar_c,1); //绘制加载条块
        }
    }
    CTBCanvas_Output(); //输出缓冲画布
}
...
function CT_LoadBarShowError(c,s)
{
    ...
    if(CT_LOADBAR_SHOW === 1) //如果加载条显示
    {
        ...
        {
            CTBCanvas_Clear(); //清除画布
            CTBCanvas_String(err_x,err_y,16,err_c,"ERROR : The resource file is not found : " + s); //显示错误信息
            CTBCanvas_Output(); //输出缓冲画布
        }
        if(c === -1) //如果错误代码是-1
        {
            CTBCanvas_Clear(); //清除画布
            CTBCanvas_String(err_x,err_y,16,err_c,"ERROR : Run context is local,can't load text file : " + s); //显示错误信息
            CTBCanvas_Output(); //输出缓冲画布
        }
    }
}
