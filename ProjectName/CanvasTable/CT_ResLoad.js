                        ...
                        xmlhttp.onreadystatechange = function() //ajax状态
                        {
                            if(xmlhttp.readyState === 4)
                            {
                                if(xmlhttp.status === 200) //如果成功读取文本文件
                                {
                                    ...
                                    var line = CT_TextLineNumber(CT_ResIndex);
                                    //将文本参数存入资源列表
...
// 计算文本行数
//--------------------------------------------------------------------
function CT_TextLineNumber(ri)
{
    var line = 1; //文本数据行数
    var td_len = CT_ContainerResource[ri].length; //文本数据长度
    var i;
    for(i=0;i<td_len;i++) //遍历文本数据
    {
        var tc = CT_ContainerResource[ri][i]; //获取文本数据当前字符
        if(tc === "\n") //如果当前字符是换行符
        {
            line ++; //行数 +1
        }
