...
// 加载资源
//--------------------------------------------------------------------
function CT_ResourceLoad(rl)
{
    if(rl.length !== 0) //如果资源列表参数长度不为0
    {
        var res_name = CT_PathGetName(rl[CT_ResIndex]); //资源文件名
            ...
            if(res_name.lastIndexOf(".") !== 0) //如果资源文件名中"."的位置不为0
            {
                ...
                if(res_extname === "png" || res_extname === "jpg" || res_extname === "jpeg" || res_extname === "bmp" || res_extname === "mp3" || res_extname === "ogg") //如果资源文件类型是支持的
                {
                    if(res_extname === "png" || res_extname === "jpg" || res_extname === "jpeg"|| res_extname === "bmp") //如果资源文件是图像
                    {
                        ...
                        image.src = rl[CT_ResIndex]; //资源列表中的文件路径
                        image.onload = function() //如果图像加载完成
                        {
                            ...
                            CT_ResourceList[CT_ResIndex] = [rl[CT_ResIndex],0,image_w,image_h]; //将图像路径存入资源列表
                            cprint("Resource load : " + rl[CT_ResIndex]);
                            CT_loadBarWidget(); //加载条循环
                            CT_ResIndex ++; //资源索引 +1
                            if(CT_ResIndex  === rl.length) //如果资源加载完成
                            {
                                CT_ResourceLoadEnd(); //结束加载资源
                            }
                            else //如果资源没有加载完
                            {
                                CT_ResourceLoad(rl); //回调，加载下一个资源
                            }
                        };
                        image.onerror = function() //如果图像未找到
                        {
                            CT_LoadBarShowError(0,rl[CT_ResIndex]); //显示错误信息
                        };
                    }
                    if(res_extname === "mp3" || res_extname === "ogg") //如果资源文件是音频
                    {
                        var audio = new Audio(); //新建音频
                        audio.src = rl[CT_ResIndex]; //资源列表中当前音频路径
                        CT_ContainerResource[CT_ResIndex] = audio; //将音频存入资源容器
                        CT_ResourceList[CT_ResIndex] = [rl[CT_ResIndex],1]; //将音频路径存入资源列表
                        cprint("Resource load : " + rl[CT_ResIndex]);
                        CT_loadBarWidget(); //加载条循环
                        CT_ResIndex ++; //资源索引 +1
                        if(CT_ResIndex === rl.length) //如果资源加载完成
                ...
                {
                    cprint("ERROR 3 : The resource file type is not supoort : " + rl[CT_ResIndex]);
                }
            }
            else
            {
                cprint("ERROR 2 : The resource file name is error : " + rl[CT_ResIndex]);
            }
        }
        else
        {
            cprint("ERROR 1 : The resource file name is not support : " + rl[CT_ResIndex]);
        }
    }
    else //如果资源列表长度为0
    {
