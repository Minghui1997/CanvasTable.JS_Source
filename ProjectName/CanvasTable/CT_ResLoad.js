...
function CT_ResourceLoad()
{
    if(CT_ResourceList.length !== 0) //如果资源列表参数长度不为0
    {
        ...
        if(res_name.lastIndexOf(".") !== -1) //如果资源文件名中有"."
        {
            ...
            {
                cprint("ERROR : The resource file name is error : " + CT_ResourceList[CT_ResIndex][0]);
            }
        }
        else
        {
            cprint("ERROR : The resource file name is not support : " + CT_ResourceList[CT_ResIndex][0]);
        }
    }
    else //如果资源列表长度为0
    {
