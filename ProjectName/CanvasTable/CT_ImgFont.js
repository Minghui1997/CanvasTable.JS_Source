...
// 绘制图像字符串(缓冲)
//--------------------------------------------------------------------
function CTBCanvas_ImgString(x,y,a,str,font_img,font_number,font_gap,font_charlist,font_glyphlist)
{
    var _char = ""; //当前字符
    ...
    while(str_i < str_l) //遍历字符串
    {
        ...
        while(tree_end < 2)
        {
            if(font_charlist[tree_li] === _char) //如果在树的左节点中找到当前字符
            {
                ...
                CTBCanvas_Glyph(cx,y,font_img,gx,gy,gw,gh,a); //绘制字体字形
...
// 绘制字体字形(缓冲)
//--------------------------------------------------------------------
function CTBCanvas_Glyph(x,y,i,ix,iy,iw,ih,a)
{
    CT_BCtx.globalAlpha = a / 255; //设置全局透明度
    CT_BCtx.drawImage(CT_ContainerResource[i],ix,iy,iw,ih,x,y,iw,ih); //将字形绘制到混合画布
    CT_BCtx.globalAlpha = 1; //还原全局透明度
}
//--------------------------------------------------------------------
//--------------------------------------------------------------------
// 测量图像字符串宽度
