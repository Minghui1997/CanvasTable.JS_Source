...
function mouse_wheel(mkey)
{
    ...
    {
        cprint("ERROR : The mouse wheel direction is error.");
    }
    return 0;
}
//--------------------------------------------------------------------
...
{
    var data_i = id * 7; //触摸键数据索引
    if(CT_ContainerTouchKey[data_i] !== 1) //如果指定索引的触摸键为空
    {
        if(w > 0 && h > 0 && key >= 0 && key <= 255) //如果触摸键属性正确
        {
            //将触摸键数据存入指定索引中
            CT_ContainerTouchKey[data_i] = 1;
            CT_ContainerTouchKey[data_i+1] = x;
            CT_ContainerTouchKey[data_i+2] = y;
            CT_ContainerTouchKey[data_i+3] = w;
            CT_ContainerTouchKey[data_i+4] = h;
            CT_ContainerTouchKey[data_i+5] = key;
            CT_ContainerTouchKey[data_i+6] = -1;
        }
        else //如果按键键值不在范围内
        {
...
{
    var data_i = id * 7; //触摸键数据索引
    if(CT_ContainerTouchKey[data_i] === 1) //如果指定索引的触摸键不为空
    {
        if(CT_ContainerTouchKey[data_i+6] !== -1) //如果触摸键有图像
        {
            CT_Repaint = 1; //请求重绘
        }
        var i;
        for(i=0;i<7;i++) //遍历触摸键数据
        {
            CT_ContainerTouchKey[data_i+i] = 0; //清除触摸键数据
        }
    }
    else //如果触摸键容器指定索引为空
    {
...
function touchkey_image(id,img)
{
    ...
    if(CT_ContainerTouchKey[data_i] === 1) //如果指定索引的触摸键不为空
    {
        if(img !== -1) //如果资源不为空
        {
            if(CT_ResourceArgList[img*5+1] === 0) //如果资源类型是图像
            {
                CT_ContainerTouchKey[data_i+6] = img; //设置触摸键图像
                CT_Repaint = 1; //请求重绘
            }
        }
    }
