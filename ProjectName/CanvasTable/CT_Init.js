...
function CT_init(cw,ch,rl,main,loop,dm,sph,lbh,arg4,arg5,arg6,arg7,arg8,arg9,arg10,arg11,arg12,arg13,arg14,arg15,arg16)
{
    ...
    CT_Main = main; //Main函数
    CT_Loop = loop; //Loop函数
    cprint("CanvasTable.JS Version : 1.X");
    cprint("CanvasTable.JS Build : 2023/01/18");
    cprint("----------Init----------");
    CT_PreferSet(); //设置偏好
    CT_CanvasCreate(); //创建主画布
    CT_BufferCanvasCreate(); //创建缓冲画布
    CT_BlendCanvasCreate(); //创建混合画布
    cprint("------------------------");
...
function CT_PreferSet()
{
    ...
    CT_BROWSER = CT_BrowserCheck(); //浏览器判断
    CT_DEVICE = CT_DeviceCheck(); //设备判断
    CT_RUN_CONTEXT = CT_RunContextCheck(); //运行环境判断
...
// 创建混合画布
//--------------------------------------------------------------------
function CT_BlendCanvasCreate()
{
    CT_BlendCanvas = document.createElement("canvas"); //创建混合画布
    CT_BlendCanvas.width = 1; //混合画布宽度
    CT_BlendCanvas.height = 1; //混合画布高度
    CT_BlCtx = CT_BlendCanvas.getContext("2d"); //混合画布绘图环境
}
//--------------------------------------------------------------------