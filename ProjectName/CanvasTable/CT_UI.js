...
// 
// [UI数据,...]
// 按钮:[id,对应窗口的索引,1,x,y,宽度,高度,背景颜色,上边框颜色,下边框颜色,左边框颜色,右边框颜色,文本颜色,背景图片,文本,文本宽度,文本高度,点击状态,始终按下状态,按下状态,松开状态,图像偏移]
...
// 文本框:[id,对应窗口的索引,6,x,y,宽度,高度,背景颜色,上边框颜色,下边框颜色,左边框颜色,右边框颜色,字体颜色,背景图片,值,密码,数字,最大字符数,只读,子文本框,子文本框名称,字体大小,图像偏移]
//--------------------------------------------------------------------
var CT_UIId = 0;
...
function CT_UIRun()
{
    ...
    if(CT_Textbox_Index !== -1) //如果点击了文本框
    {
        if(CT_ContainerUI[CT_Textbox_Index] != null) //如果点击的文本框不为空
        {
            ...
            if(readonly === 0) //如果文本框不是只读
            {
                if(CT_KeyChar !== "" && CT_KeyChar !== -1) //如果输入了字符
                {
    ...
    for(i=0;i<CT_ContainerWindowIndex;i++) //遍历窗口容器
    {
        ...
        if(CT_ContainerWindow[i] != null) //如果当前窗口不为空
        {
            ...
            if(window_layer === l) //如果当前窗口层等于渲染层
            {
                ...
                {
                    CTBCanvas_Rect(win_x,win_y,win_w,win_h,win_cbg,1); //绘制窗口背景
                    CTBCanvas_Rect(win_x,win_y-2,win_w,2,win_cbu,1); //绘制窗口上边框
                    CTBCanvas_Rect(win_x,win_y+win_h,win_w,2,win_cbd,1); //绘制窗口下边框
                    CTBCanvas_Rect(win_x-2,win_y-2,2,win_h+4,win_cbl,1); //绘制窗口左边框
                    CTBCanvas_Rect(win_x+win_w,win_y-2,2,win_h+4,win_cbr,1); //绘制窗口右边框
                }
                var i2;
                for(i2=0;i2<CT_ContainerUIIndex;i2++) //遍历UI容器
                {
                    if(CT_ContainerUI[i2] != null) //如果当前UI对象不为空
                    {
                        ...
                        if(ui_win_index === win_index) //如果当前UI对象对应窗口索引等于当前窗口索引
                        {
                            ...
                            if(ui_type === 1) //如果当前UI对象是按钮
                            {
                                ...
                                {
                                    CTBCanvas_Rect(b_x,b_y,b_w,b_h,b_cbg,1); //绘制按钮背景
                                    CTBCanvas_Rect(b_x,b_y,b_w,2,b_cbu,1); //绘制按钮上边框
                                    CTBCanvas_Rect(b_x,b_y+b_h-2,b_w,2,b_cbd,1); //绘制按钮下边框
                                    CTBCanvas_Rect(b_x,b_y,2,b_h-2,b_cbl,1); //绘制按钮左边框
                                    CTBCanvas_Rect(b_x+b_w-2,b_y,2,b_h-2,b_cbr,1); //绘制按钮右边框
                                }
                            ...
                            if(ui_type === 3)
                            {
                                ...
                                var c_s = CT_ContainerUI[i2][10]; //复选框状态
                                CTBCanvas_Rect(c_x,c_y,c_w,c_h,c_cbg,1); //绘制复选框背景
                                CTBCanvas_Rect(c_x,c_y,c_w,c_h,c_cb,0); //绘制复选框外边框
                                CTBCanvas_Rect(c_x+1,c_y+1,c_w-2,c_h-2,c_cb,0); //绘制复选框内边框
                                if(c_s === 1) //如果复选框状态为１
                                {
                            ...
                            if(ui_type === 4)
                            {
                                ...
                                CTBCanvas_Circle(r_x+8,r_y+8,r_w/2-2,r_cgb,1); //绘制单选框边框
                                if(r_s === 1) //如果单选框状态为１
                                {
                            ...
                            if(ui_type === 5)
                            {
                                ...
                                {
                                    CTBCanvas_Rect(p_x,p_y,p_w,p_h,p_cgb,1); //绘制进度条背景
                                    CTBCanvas_Rect(p_x,p_y,p_w,2,p_cbu,1); //绘制进度条上边框
                                    CTBCanvas_Rect(p_x,p_y+p_h-2,p_w,2,p_cbd,1); //绘制进度条下边框
                                    CTBCanvas_Rect(p_x,p_y,2,p_h-2,p_cbl,1); //绘制进度条左边框
                                    CTBCanvas_Rect(p_x+p_w-2,p_y,2,p_h-2,p_cbr,1); //绘制进度条右边框
                                }
                                CTBCanvas_Rect(p_x+4,p_y+4,p_ww,p_h-8,p_cw,1); //绘制进度条部件
                            }
                            if(ui_type === 6)
                            {
                                ...
                                {
                                    CTBCanvas_Rect(t_x,t_y,t_w,t_h,t_cbg,1); //绘制文本框背景
                                    CTBCanvas_Rect(t_x,t_y,t_w,2,t_cbu,1); //绘制文本框上边框
                                    CTBCanvas_Rect(t_x,t_y+t_h-2,t_w,2,t_cbd,1); //绘制文本框下边框
                                    CTBCanvas_Rect(t_x,t_y,2,t_h-2,t_cbl,1); //绘制文本框左边框
                                    CTBCanvas_Rect(t_x+t_w-2,t_y,2,t_h-2,t_cbr,1); //绘制文本框右边框
                                }
                                if(t_pass !== 1) //如果文本框不是密码文本框
                                {
                                    var text_y = t_y + (t_h - t_fsize) / 2; //文本框字符串位置y
                                    CTBCanvas_String(t_x+5,text_y,t_fsize,t_ct,t_val); //绘制文本框字符串
                                }
                                else //如果文本框是密码文本框
                                {
                                    ...
                                    CTBCanvas_String(t_x+5,t_y+3,t_fsize,t_ct,p); //绘制文本框密码串
                                }
                                if(i2 === CT_Textbox_Index) //如果选择的文本框是当前文本框
                                {
                                    ...
                                    {
                                        CTBCanvas_Line(t_x+4+val_width+1,t_y+3,t_x+4+val_width+1,t_y+t_h-4,1,color(0,0,0,255)); //绘制文本框输入线
                                    }
                                }
                            }
                        }
