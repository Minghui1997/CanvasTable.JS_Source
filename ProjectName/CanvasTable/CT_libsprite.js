        ...
        if(CTsprite.type === -2) //如果对象类型是sprite
        {
            ...
            if(image !== -1) //如果sprite有图像
            {
                ...
                if(res_type === 0) //如果资源类型是图像
                {
                    ...
                    if(box_cx === 0 && box_cy === 0 && box_r === 0) //如果没有设置圆形碰撞盒
                    {
                        ...
                        box_r = parseInt(subimg_w / 2); //sprite碰撞盒宽度/半径
                    }
                }
                else
                {
            ...
            if(subnum >= 1 && subimg <= subnum - 1) //如果sprite属性正确
            {
                ...
                for(i=0;i<CT_ContainerSpriteLength;i++) //遍历sprite容器
                {
                    ...
                    {   
                        CT_ContainerSprite[i] = [id,layer,x,y,image,subimg,subnum,subimg_w,subimg_h,x,y,coll_t,coll_e,subimg_o,box_x,box_y,box_w,box_h,box_cx,box_cy,box_r]; //将sprite数据写入sprite容器
