                ...
                if(subimg <= subnum - 1) //如果子图像索引小于最大
                {
                    if(CT_ContainerSpriteIndex > 0) //如果sprite容器长度大于0
                    {
                        ...
                        for(i=0;i<CT_ContainerSpriteIndex;i++) //遍历sprite容器
                        {
                            ...
                            {
                                CT_ContainerSprite[i] = [id,layer,x,y,image,subimg,subnum,subimg_w,subimg_h,x,y,coll_t,coll_e,subimg_o]; //将sprite数据写入sprite容器
                                CT_TaskCreated(i,CTsprite.main,CTsprite.loop); //创建sprite程序
                                CT_InterfaceRepaint(); //请求重绘
                                return i; //中断
                           }
                        }
                        CT_ContainerSprite[conspr_i] = [id,layer,x,y,image,subimg,subnum,subimg_w,subimg_h,x,y,coll_t,coll_e,subimg_o]; //将sprite数据写入sprite容器
                    ...
                    {
                        CT_ContainerSprite[conspr_i] = [id,layer,x,y,image,subimg,subnum,subimg_w,subimg_h,x,y,coll_t,coll_e,subimg_o]; //将sprite数据sprite容器
                        CT_TaskCreated(conspr_i,CTsprite.main,CTsprite.loop); //创建sprite程序
                        CT_ContainerSpriteIndex ++; //sprite容器索引 +1
                        CT_InterfaceRepaint(); //请求重绘
                        return conspr_i; //返回sprite索引
