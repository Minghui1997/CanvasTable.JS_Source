        ...
        if(CTwindow.type === 0) //如果对象类型是窗口
        {
            ...
            if(w > 0 && h > 0) //如果窗口对象属性正确
            {
                ...
                for(i=0;i<CT_ContainerWindowLength;i++) //遍历窗口容器
                {
                    ...
                    {
                        var win_id = CT_ContainerWindow[i][0]; //获取当前窗口id
    ...
    if(CT_RunnerStatus === 1) //如果运行器状态为1
    {
        if(CTwindow.type === 0) //如果对象类型是窗口
        {
            ...
            for(i=0;i<CT_ContainerUILength;i++) //遍历UI容器
            {
                ...
                {
                    var ui_id = CT_ContainerUI[i][0]; //获取当前UI id
            ...
            if(type === 1) //如果UI是按钮
            {
                ...
                var font_n = 0; //字体字符数
                var font_g = 0; //字体字符间隙
                var font_cl = null; //字体字符列表
                var font_gl = null; //字体字形列表
                ...
                if(w > 0 && h > 0) //如果按钮属性正确
                {
                    ...
                    for(i2=0;i2<CT_ContainerUILength;i2++) //遍历UI容器
                    {
                        ...
                        {
                            CT_ContainerUI[i2] = [id,win_index,type,x,y,w,h,cbg,cbu,cbd,cbl,cbr,ct,bi,text,text_w,text_h,0,0,0,0,bio,imgfont_use,font_i,font_n,font_g,font_cl,font_gl]; //将UI数据写入UI容器
                            CTui.index = i2; //UI对象索引
                            CT_Repaint = 1; //请求重绘
                            return; //中断
                        }
                    }
                    CT_ContainerUI[CT_ContainerUILength] = [id,win_index,type,x,y,w,h,cbg,cbu,cbd,cbl,cbr,ct,bi,text,text_w,text_h,0,0,0,0,bio,imgfont_use,font_i,font_n,font_g,font_cl,font_gl]; //将UI数据写入UI容器
            ...
            if(type === 2) //如果UI是文本
            {
                ...
                for(i2=0;i2<CT_ContainerUILength;i2++) //遍历UI容器
                {
                    if(CT_ContainerUI[i2] == null) //如果当前UI为空
                    {
                        CT_ContainerUI[i2] = [id,win_index,type,x,y,ct,text,ts,imgfont_use,font_i,font_n,font_g,font_cl,font_gl]; //将UI数据写入UI容器
                        CTui.index = i2; //UI对象索引
                        CT_Repaint = 1; //请求重绘
                        return; //中断
                    }
                }
                CT_ContainerUI[CT_ContainerUILength] = [id,win_index,type,x,y,ct,text,ts,imgfont_use,font_i,font_n,font_g,font_cl,font_gl]; //将UI数据写入UI容器
                CTui.index = CT_ContainerUILength; //UI对象索引
                CT_ContainerUILength ++; //UI容器索引 +1
                CT_Repaint = 1; //请求重绘
            }
            ...
            if(type === 6) //如果UI是文本框
            {
                ...
                if(CTui.imgfont != null) //如果使用图像字体
                {
                    if(CTui.imgfont.FontImg !== -1 && CTui.imgfont.CharNumber > 0 && CTui.imgfont.CharList != null && CTui.imgfont.GlyphList != null) //如果资源不为空
                    {
                        ...
                        font_cl = CTui.imgfont.CharList; //获取字体字符列表
                        font_gl = CTui.imgfont.GlyphList; //获取字体字形列表
                        size = CTui.imgfont.CharHeight; //按钮显示文本高度
                        imgfont_use = 1; //使用图像字体
                    }
                    else
                    {
                ...
                if(w > 0 && h > 0 && tpass >= 0 && tnumb >= 0 && tmax > 0) //如果文本框属性正确
                {
                    ...
                    for(i2=0;i2<CT_ContainerUILength;i2++) //遍历UI容器
                    {
                        if(CT_ContainerUI[i2] == null) //如果当前UI为空
                        {
                            CT_ContainerUI[i2] = [id,win_index,type,x,y,w,h,tbg,tbu,tbd,tbl,tbr,tc,tb,"",tpass,tnumb,tmax,readonly,subbox,name,size,tb_o,imgfont_use,font_i,font_n,font_g,font_cl,font_gl]; //将UI数据写入UI容器
                            CTui.index = i2; //UI对象索引
                            CT_Repaint = 1; //请求重绘
                            return; //中断
                        }
                    }
                    CT_ContainerUI[CT_ContainerUILength] = [id,win_index,type,x,y,w,h,tbg,tbu,tbd,tbl,tbr,tc,tb,"",tpass,tnumb,tmax,readonly,subbox,name,size,tb_o,imgfont_use,font_i,font_n,font_g,font_cl,font_gl]; //将UI数据写入UI容器
                    CTui.index = CT_ContainerUILength; //UI对象索引
                    CT_ContainerUILength ++; //UI容器索引 +1
                    CT_Repaint = 1; //请求重绘
                }
            ...
            if(type === 7) //如果UI是图像元件
            {
                ...
                var subimg_o = CTui.subimg_o; //图像元件子图像偏移
                ...
                if(image !== -1) //如果图像元件有图像
                {
                    ...
                    if(res_type === 0) //如果资源类型是图像
                    {
                        image_w = CT_ResourceList[image][2]; //图像宽度
                        image_h = CT_ResourceList[image][3]; //图像高度
                        subimg_w = parseInt(image_w / subnum - subimg_o * 2); //子图像宽度
                    ...
                    if(subnum >= 1 && subimg <= subnum - 1) //如果图像元件属性正确
                    {
                        ...
                        for(i2=0;i2<CT_ContainerUILength;i2++) //遍历UI容器
                        {
                            if(CT_ContainerUI[i2] == null) //如果当前UI为空
                            {
                                CT_ContainerUI[i2] = [id,win_index,type,x,y,image,subimg,subnum,subimg_w,subimg_h,subimg_o]; //将UI数据写入UI容器
                                CTui.index = i2; //UI对象索引
                                CT_Repaint = 1; //请求重绘
                                return; //中断
                            }
                        }
                        CT_ContainerUI[CT_ContainerUILength] = [id,win_index,type,x,y,image,subimg,subnum,subimg_w,subimg_h,subimg_o]; //将UI数据写入UI容器
                        CTui.index = CT_ContainerUILength; //UI对象索引
                        CT_ContainerUILength ++; //UI容器索引 +1
                        CT_Repaint = 1; //请求重绘
                    }
            ...
            if(CT_ContainerUI[CTui.index] != null) //如果UI容器中指定UI对象的数据不为空
            {
                ...
                if(CTui.id === Cui_id) //如果UI对象id等于容器中指定UI对象的ID
                {
                    if(CTui.type === 1) //如果UI对象是按钮
                    {
                        ...
                        {
                            var str = CT_ContainerUI[CTui.index][14]; //按钮显示文本
                            var str_w = CT_ImgStringMeasure(imgfont.CharNumber,imgfont.CharGap,imgfont.CharList,imgfont.GlyphList,str); //测量图像字符串像素宽度
                            //将图像字体变量写入UI容器
                            CT_ContainerUI[CTui.index][15] = str_w;
                            CT_ContainerUI[CTui.index][16] = imgfont.CharHeight;
                            CT_ContainerUI[CTui.index][22] = 1;
                            CT_ContainerUI[CTui.index][23] = imgfont.FontImg;
                            CT_ContainerUI[CTui.index][24] = imgfont.CharNumber;
                            CT_ContainerUI[CTui.index][25] = imgfont.CharGap;
                            CT_ContainerUI[CTui.index][26] = imgfont.CharList;
                            CT_ContainerUI[CTui.index][27] = imgfont.GlyphList;
                            CT_Repaint = 1; //请求重绘
                        }
                        else
                        {
                            ...
                            //将一般字体变量写入UI容器
                            CT_ContainerUI[CTui.index][15] = str_w;
                            CT_ContainerUI[CTui.index][16] = CTui.size;
                            //清除图像字体变量
                            CT_ContainerUI[CTui.index][22] = 0;
                            CT_ContainerUI[CTui.index][23] = 0;
                            CT_ContainerUI[CTui.index][24] = 0;
                            CT_ContainerUI[CTui.index][25] = 0;
                            CT_ContainerUI[CTui.index][26] = null;
                            CT_ContainerUI[CTui.index][27] = null;
                            CT_Repaint = 1; //请求重绘
                        }
                    }
                    if(CTui.type === 2) //如果UI对象是文本
                    {
                        ...
                        {
                            //将图像字体变量写入UI容器
                            CT_ContainerUI[CTui.index][8] = 1;
                            CT_ContainerUI[CTui.index][9] = imgfont.FontImg;
                            CT_ContainerUI[CTui.index][10] = imgfont.CharNumber;
                            CT_ContainerUI[CTui.index][11] = imgfont.CharGap;
                            CT_ContainerUI[CTui.index][12] = imgfont.CharList;
                            CT_ContainerUI[CTui.index][13] = imgfont.GlyphList;
                            CT_Repaint = 1; //请求重绘
                        }
                        else
                        {
                            //清除图像字体变量
                            CT_ContainerUI[CTui.index][8] = 0;
                            CT_ContainerUI[CTui.index][9] = 0;
                            CT_ContainerUI[CTui.index][10] = 0;
                            CT_ContainerUI[CTui.index][11] = 0;
                            CT_ContainerUI[CTui.index][12] = null;
                            CT_ContainerUI[CTui.index][13] = null;
                            CT_Repaint = 1; //请求重绘
                        }
                    }
                    if(CTui.type === 6) //如果UI对象是文本框
                    {
                        ...
                        {
                            //将图像字体变量写入UI容器
                            CT_ContainerUI[CTui.index][23] = 1;
                            CT_ContainerUI[CTui.index][24] = imgfont.FontImg;
                            CT_ContainerUI[CTui.index][25] = imgfont.CharNumber;
                            CT_ContainerUI[CTui.index][26] = imgfont.CharGap;
                            CT_ContainerUI[CTui.index][27] = imgfont.CharList;
                            CT_ContainerUI[CTui.index][28] = imgfont.GlyphList;
                        }
                        else
                        {
                            //清除图像字体变量
                            ...
                            CT_ContainerUI[CTui.index][24] = 0;
                            CT_ContainerUI[CTui.index][25] = 0;
                            CT_ContainerUI[CTui.index][26] = 0;
                            CT_ContainerUI[CTui.index][27] = null;
                            CT_ContainerUI[CTui.index][28] = null;
                        }
                    }
                }
            }
...
function imagement_set_image(CTui,img,subnum)
{
    if(CT_RunnerStatus === 1) //如果运行器状态为1
    {
        ...
        {
            if(CT_ContainerUI[CTui.index] != null) //如果UI容器中指定图像元件对象的数据不为空
            {
                var Cui_id = CT_ContainerUI[CTui.index][0]; //UI容器中指定UI对象的ID
                if(CTui.id === Cui_id) //如果UI对象id等于容器中指定UI对象的ID
                {
                    ...
                    if(res_type === 0) //如果资源类型是图像
                    {
                        var image_w = CT_ResourceList[img][2]; //图像元件图像宽度
                        var image_h = CT_ResourceList[img][3]; //图像元件图像高度
                        var subimg_w = 0; //图像元件子图像宽度
                        var subimg_h = 0; //图像元件子图像高度
                        var subimg_o = CT_ContainerUI[CTui.index][10]; //子图像偏移
                        ...
                        //将图像元件数据写入UI容器
                        CT_ContainerUI[CTui.index][5] = img;
                        CT_ContainerUI[CTui.index][6] = 0;
                        CT_ContainerUI[CTui.index][7] = subnum;
                        CT_ContainerUI[CTui.index][8] = subimg_w;
                        CT_ContainerUI[CTui.index][9] = subimg_h;
                        CT_Repaint = 1; //请求重绘
                    }
                    else
                    {
    ...
    if(CT_RunnerStatus === 1) //如果运行器状态为1
    {
        if(CTui.type === 7) //如果UI对象是图像元件
        {
            if(CT_ContainerUI[CTui.index] != null) //如果UI容器中指定图像元件对象的数据不为空
            {
                ...
                if(CTui.id === Cui_id) //如果UI对象id等于容器中指定UI对象的ID
                {
                    var subnum = CT_ContainerUI[CTui.index][7]; //图像元件子图像数量
                    if(subnum >= 1 && i <= subnum - 1) //如果图像元件参数正确
                    {
                        CT_ContainerUI[CTui.index][6] = i; //设置图像元件子图像
                        CT_Repaint = 1; //请求重绘
                    }
                    else
                    {
