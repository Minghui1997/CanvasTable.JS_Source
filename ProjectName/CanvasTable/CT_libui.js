...
function CTwindow_create(CTwindow)
{
    if(CTwindow.type === 0) //如果对象类型是窗口
    {
        ...
        {
            if(CT_ContainerWindow.length > 0) //如果窗口容器长度大于0
            {
                var i2;
                for(i2=0;i2<CT_ContainerWindow.length;i2++) //遍历窗口容器
                {
...
function CTwindow_destroy(CTwindow)
{
    if(CTwindow.type === 0) //如果对象类型是窗口
    {
        ...
        var i;
        for(i=0;i<CT_ContainerUI.length;i++) //遍历UI对象容器
        {
...
function CTui_create(CTwindow,CTui)
{
    if(CTwindow.type === 0) //如果对象类型是窗口
    {
        ...
        if(type === 3) //如果UI是复选框
        {
            ...
            var state = CTui.state; //复选框状态
            if(state === 0 || state === 1) //如果复选框状态正确
            {
                if(CT_ContainerUI.length > 0) //如果UI容器长度大于0
                {
                    var i2;
                    for(i2=0;i2<CT_ContainerUI.length;i2++) //遍历UI容器
                    {
                        if(CT_ContainerUI[i2] == null) //如果当前UI为空
                        {
                            CT_ContainerUI[i2] = [id,win_index,type,x,y,w,h,cbg,cb,cw,state]; //将UI数据写入UI容器
                            CTui.index = i2; //UI对象索引
                            CT_InterfaceRepaint(); //请求重绘
                            return; //中断
                        }
                    }
                    CT_ContainerUI[CT_ContainerUIIndex] = [id,win_index,type,x,y,w,h,cbg,cb,cw,state]; //将UI数据写入UI容器
                    CTui.index = CT_ContainerUIIndex; //UI对象索引
                    CT_ContainerUIIndex ++; //UI容器索引 +1
                    CT_InterfaceRepaint(); //请求重绘
                }
            ...
            {
                cprint("ERROR 121 : Radio state error. UIobjectID : " + String(id));
            }
        }
        if(type === 4) //如果UI是单选框
        {
