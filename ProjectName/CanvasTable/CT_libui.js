    ...
    if(CT_RunnerStatus === 1) //如果运行器状态为1
    {
        if(CTwindow.type === 0) //如果对象类型是窗口
        {
            ...
            if(bi !== -1) //如果有背景图像
            {
                ...
                {
                    bi = -1; //窗口无背景图像
                    cprint("ERROR 4 : The resource index corresponding to the resource file is not a image. ResIndex : " + String(bi));
                }
            }
        ...
        if(CTwindow.type === 0) //如果对象类型是窗口
        {
            ...
            if(type === 5) //如果UI是进度条
            {
               var x = CTui.x; //进度条位置x
                ...
                if(pi !== -1) //如果有背景图像
                {
                    ...
                    {
                        pi = -1; //窗口无背景图像
                        cprint("ERROR 4 : The resource index corresponding to the resource file is not a image. ResIndex : " + String(pi));
                    }
                }
            ...
            if(type === 6) //如果UI是文本框
            {
                ...
                if(tb !== -1) //如果有背景图像
                {
                    ...
                    {
                        tb = -1; //窗口无背景图像
                        cprint("ERROR 4 : The resource index corresponding to the resource file is not a image. ResIndex : " + String(tb));
                    }
                }
                            ...
                            if(tmax > 0) //如果文本框最大字符数大于0
                            {
                                if(CT_ContainerUIIndex > 0) //如果UI容器长度大于0
                                {
                                    ...
                                    for(i2=0;i2<CT_ContainerUIIndex;i2++) //遍历UI容器
                                    {
                                        if(CT_ContainerUI[i2] == null) //如果当前UI为空
                                        {
                                            CT_ContainerUI[i2] = [id,win_index,type,x,y,w,h,tbg,tbu,tbd,tbl,tbr,tc,tb,"",tpass,tnumb,tmax,readonly,subbox,name,size]; //将UI数据写入UI容器
                                            CTui.index = i2; //UI对象索引
                                            CT_InterfaceRepaint(); //请求重绘
                                            return; //中断
                                        }
                                    }
                                    CT_ContainerUI[CT_ContainerUIIndex] = [id,win_index,type,x,y,w,h,tbg,tbu,tbd,tbl,tbr,tc,tb,"",tpass,tnumb,tmax,readonly,subbox,name,size]; //将UI数据写入UI容器
                                    CTui.index = CT_ContainerUIIndex; //UI对象索引
                                    CT_ContainerUIIndex ++; //UI容器索引 +1
                                    CT_InterfaceRepaint(); //请求重绘
                                }
                ...
                if(CTui.id === Cui_id) //如果UI对象id等于容器中指定UI对象的ID
                {
                    if(CTui.type === 1 || CTui.type === 5 || CTui.type === 6) //如果UI对象是按钮 进度条 文本框
                    {
                        if(img !== -1) //如果有背景图像
                        {
                            ...
                            {
                                cprint("ERROR 4 : The resource index corresponding to the resource file is not a image. ResIndex : " + String(img));
                            }
                        }
                        else
                        {
    ...
    if(CT_RunnerStatus === 1) //如果运行器状态为1
    {
        if(CTui.type === 6) //如果UI对象是文本框
        {
            if(CT_ContainerUI[CTui.index] != null) //如果UI容器中指定文本框对象的数据不为空
            {
                ...
                if(CTui.id === Cui_id) //如果UI对象id等于容器中指定UI对象的ID
                {
                    var str_l = str.length; //设置的字符串长度
                    if(str_l <= CTui.max) //如果设置的字符串长度小于等于文本框最大字符数
                    {
                        if(CTui.number === 0) //如果文本框不是数字文本框
                        {
                        ...
                        {
                            var i;
                            for(i=0;i<str_l;i++) //遍历设置的字符串中的每个字符
                            {
                                if(str[i] !== "0" && str[i] !== "1" && str[i] !== "2" && str[i] !== "3" && str[i] !== "4" && str[i] !== "5" && str[i] !== "6" && str[i] !== "7" && str[i] !== "8" && str[i] !== "9") //如果当前字符不是数字
                                {
                                    cprint("ERROR 122 : The textbox is number textbox. UIobjectID : " + String(CTui.id));
                                    return; //中断
                                }
                            }
                            CT_ContainerUI[CTui.index][14] = str; //设置文本框内容
                    ...
                    {
                        cprint("ERROR 123 : Char number of textbox greater than the max. UIobjectID : " + String(CTui.id));
                    }
                }
            }
        }
