        ...
        if(CTwindow.type === 0) //如果对象类型是窗口
        {
            ...
            if(w > 0 && h > 0) //如果窗口对象大小正确
            {
                if(CT_ContainerWindowIndex > 0) //如果窗口容器长度大于0
                {
                    ...
                    CT_ContainerWindow[CT_ContainerWindowIndex] = [id,l,x,y,w,h,cbg,cbu,cbd,cbl,cbr,bi,bio]; //将窗口数据写入窗口容器
            ...
            if(type === 1) //如果UI是按钮
            {
                ...
                if(w > 0 && h > 0) //如果按钮大小正确
                {
                    if(CT_ContainerUIIndex > 0) //如果UI容器长度大于0
                    {
                        ...
                        CT_ContainerUI[CT_ContainerUIIndex] = [id,win_index,type,x,y,w,h,cbg,cbu,cbd,cbl,cbr,ct,bi,text,text_w,text_h,0,0,0,0,bio]; //将UI数据写入UI容器
                    ...
                    if(val >= 0 && maxval > 0) //如果进度条值正确
                    {
                        if(val <= maxval) //如果进度条值小于最大值
                        {
                            ...
                            if(CT_ContainerUIIndex > 0) //如果UI容器长度大于0
                            {
                                ...
                                CT_ContainerUI[CT_ContainerUIIndex] = [id,win_index,type,x,y,w,h,cbg,cbu,cbd,cbl,cbr,cw,pi,w_w,val,maxval,pio]; //将UI数据写入UI容器
                                CTui.index = CT_ContainerUIIndex; //UI对象索引
                                CT_ContainerUIIndex ++; //UI容器索引 +1
                                CT_InterfaceRepaint(); //请求重绘
                            }
                        ...
                        if(tnumb === 0 || tnumb === 1) //如果数字文本框参数正确
                        {
                            if(tmax > 0) //如果文本框最大字符数大于0
                            {
                                if(CT_ContainerUIIndex > 0) //如果UI容器长度大于0
                                {
                                    ...
                                    CT_ContainerUI[CT_ContainerUIIndex] = [id,win_index,type,x,y,w,h,tbg,tbu,tbd,tbl,tbr,tc,tb,"",tpass,tnumb,tmax,readonly,subbox,name,size,tb_o]; //将UI数据写入UI容器
                                    CTui.index = CT_ContainerUIIndex; //UI对象索引
                                    CT_ContainerUIIndex ++; //UI容器索引 +1
                                    CT_InterfaceRepaint(); //请求重绘
                                }
