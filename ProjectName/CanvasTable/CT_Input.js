...
document.addEventListener("keypress",function(event) //监听按键输入的字符
{
    if(event.which !== 0 && event.which !== 8) //如果字码不为0
    {
        CT_KeyChar = String.fromCharCode(event.which); //获取当前按键输入的字符
    }
},false);
...
{
    CT_MousePosX = parseInt(event.pageX * CT_CanvasCWM); //鼠标位置x
    CT_MousePosY = parseInt(event.pageY * CT_CanvasCHM); //鼠标位置y
},false);
...
document.addEventListener("touchstart",function(event) //监听触摸点按下
{
    ...
    CT_MousePosX = parseInt(t_x * CT_CanvasCWM); //鼠标位置x
    CT_MousePosY = parseInt(t_y * CT_CanvasCHM); //鼠标位置y
    ...
    {
        data_i = i * 7; //触摸键数据索引
        if(CT_ContainerTouchKey[data_i] === 1) //如果当前触摸键不为空
        {
            var touchkey_x = CT_ContainerTouchKey[data_i+1]; //获取当前触摸键位置x
            var touchkey_y = CT_ContainerTouchKey[data_i+2]; //获取当前触摸键位置y
            var touchkey_w = CT_ContainerTouchKey[data_i+3]; //获取当前触摸键宽度
            var touchkey_h = CT_ContainerTouchKey[data_i+4]; //获取当前触摸键高度
            var touchkey_key = CT_ContainerTouchKey[data_i+5]; //获取当前触摸键键值
            for(i2=0;i2<touches_l;i2++) //遍历触摸列表
            {
                var touch_x = parseInt(event.touches[i2].pageX * CT_CanvasCWM); //获取当前触摸点位置x
                var touch_y = parseInt(event.touches[i2].pageY * CT_CanvasCHM); //获取当前触摸点位置y
                if(touch_x >= touchkey_x && touch_y >= touchkey_y && touch_x < touchkey_x + touchkey_w && touch_y < touchkey_y + touchkey_h) //当前触摸键被触摸
                {
                    ...
                    CT_KeyChar = String.fromCharCode(touchkey_key); //当前按键输入的字符
                    break;
                }
            }
        }
...
document.addEventListener("touchend",function(event) //监听触摸点松开
{
    ...
    for(i=0;i<32;i++) //遍历触摸键容器
    {
        ...
        if(CT_ContainerTouchKey[data_i] === 1) //如果当前触摸键不为空
        {
            ...
            {
                var touch_x = parseInt(event.changedTouches[i2].pageX * CT_CanvasCWM); //获取当前触摸点位置x
                var touch_y = parseInt(event.changedTouches[i2].pageY * CT_CanvasCHM); //获取当前触摸点位置y
...
function CT_TouchKeyRender()
{
    ...
    for(i=0;i<32;i++) //遍历触摸键容器
    {
        ...
        if(CT_ContainerTouchKey[data_i] === 1) //如果当前触摸键不为空
        {
            var tx = CT_ContainerTouchKey[data_i+1]; //当前触摸键位置x
            var ty = CT_ContainerTouchKey[data_i+2]; //当前触摸键位置y
            var ti = CT_ContainerTouchKey[data_i+6]; //当前触摸键图像
            if(ti !== -1) //如果触摸键有图像
            {
