...
document.addEventListener("touchstart",function(event) //监听触摸点按下
{
    ...
    var i = 0;
    for(i;i<event.touches.length;i++) //遍历触摸列表
    {
...
document.addEventListener("touchend",function(event) //监听触摸点松开
{
    ...
    var i = 0;
    for(i;i<event.changedTouches.length;i++) //遍历触摸列表
    {
...
{
    var i;
    for(i=0;i<CT_ContainerTouchKey.length;i++) //遍历触摸键容器
    {
        if(CT_ContainerTouchKey[i] != null) //如果当前触摸键不为空
        {
            ...
            var i2;
            var press = 0;
            for(i2=0;i2<CT_TouchKeyList.length;i2++) //遍历触摸列表
            {
...
{
    if(CT_ContainerTouchKey.length !== 0) //如果触摸键容器长度不为0
    {
        var i;
        for(i=0;i<CT_ContainerTouchKey.length;i++) //遍历触摸键容器
        {
