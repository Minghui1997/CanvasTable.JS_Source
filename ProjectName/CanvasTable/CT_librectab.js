    ...
    if(name !== "")
    {
        if(name.indexOf("/") === -1 && name.indexOf("*") === -1 && name.indexOf(":") === -1 && name.indexOf("?") === -1 && name.indexOf("<") === -1 && name.indexOf(">") === -1 && name.indexOf("|") === -1) //如果记录表名称格式正确
        {
            if(CT_RectabName === "") //如果没有打开记录表
            {
                ...
                if(CT_RectabData === null) //如果记录表数据为null
                {
                    CT_RectabData = ""; //记录表数据为空
                }
                CT_RectabName = name; //记录表名称
            }
            else
            {
                cprint("ERROR : The record table is opened."); //输出错误   
            }
        }
        else
        {
            cprint("ERROR : The record table name is error."); //输出错误
        }
    }
}
//--------------------------------------------------------------------
...
function rectab_write(title,text)
{
    if(title !== "" && text !== "") //如果记录表标题和记录表数据不为空
    {
        ...
        {
            if(text.indexOf("#") === -1 && text.indexOf("=") === -1) //如果参数text没有 "&" 和 "="
            {
                if(CT_RectabData === "") //如果记录表为空
                {
                ...
                else //如果记录表不为空
                {
                    ...
                    else //如果记录表中有指定标题
                    {
                        ...
                        var new_part = title + "=" + text + "#"; //新数据段
                        CT_RectabData = CT_RectabData.replace(replace_part,new_part); //替换记录表中的数据段
                    }
                }
            }
            else
            {
                cprint("ERROR : The record table text is error."); //输出错误
            }
        }
        else //如果参数title有 "&"
        {
            cprint("ERROR : The record table title is error."); //输出错误
        }
    }
}
//--------------------------------------------------------------------
