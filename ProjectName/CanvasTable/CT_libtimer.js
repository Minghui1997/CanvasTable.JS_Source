...
function timer_set(timer,time)
{
    ...
    if(time >= 0.1) //如果定时器时间大于等于1
    {
        ...
        var time_frame = parseInt(time * 60); //时间帧
        CT_ContainerTimer[CT_ContainerTimerLength] = [timer,time_frame,time_frame,0,0]; //将定时器存入定时器容器
        CT_ContainerTimerLength ++; //定时器索引 +1
    }
    else
    {
...
function timer_set_f(timer,fn)
{
    ...
    if(fn >= 1) //如果定时器时间大于等于1
    {
        ...
        CT_ContainerTimer[CT_ContainerTimerLength] = [timer,fn,fn,0,0]; //将定时器存入定时器容器
        CT_ContainerTimerLength ++; //定时器索引 +1
    }
    else
    {
...
function globaltimer_set(timer,time)
{
    ...
    if(time >= 0.1) //如果全局定时器时间大于等于0.1
    {
        ...
        var time_frame = parseInt(time * 60); //时间帧
        CT_ContainerGlobalTimer[CT_ContainerGlobalTimerLength] = [timer,time_frame,time_frame,0,0]; //将全局定时器存入全局定时器容器
        CT_ContainerGlobalTimerLength ++; //全局定时器索引 +1
    }
    else
    {
        cprint("ERROR : The timer duration than the minimum.");
...
function globaltimer_set_f(timer,fn)
{
    ...
    if(fn >= 1) //如果全局定时器时间大于等于1
    {
        ...
        CT_ContainerGlobalTimer[CT_ContainerGlobalTimerLength] = [timer,fn,fn,0,0]; //将全局定时器存入全局定时器容器
        CT_ContainerGlobalTimerLength ++; //全局定时器索引 +1
    }
    else
    {
