...
function timer_set(timer,time)
{
    ...
    if(time >= 0.017) //如果定时器时间大于等于0.017
    {
        ...
        CT_ContainerTimer[CT_ContainerTimerIndex] = [timer,time_frame,time_frame,0,0]; //将定时器数据存入定时器表
...
function globaltimer_set(timer,time)
{
    ...
    {
        var time_frame = parseInt(time * 60); //时间帧
        CT_ContainerGlobalTimer[CT_ContainerGlobalTimerIndex] = [timer,time_frame,time_frame,0,0]; //将全局定时器数据存入全局定时器表
