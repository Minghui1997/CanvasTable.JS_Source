...
function textdata_read(t)
{
    if(t !== -1) //如果资源不为空
    {
        var res_type = CT_ResourceArgList[t*5+1]; //资源类型
        ...
        {
            cprint("ERROR : The resource index corresponding to the resource file is not text.");
        }
    }
    return "";
}
...
function textdata_line(t)
{
    if(t !== -1) //如果资源不为空
    {
        var ldata_i = t * 5; //资源参数列表的数据索引
        var res_type = CT_ResourceArgList[ldata_i+1]; //资源类型
        if(res_type === 2) //如果资源是文本
        {
            var line = 1; //文本行
            var text_l = CT_ResourceArgList[ldata_i+2]; //获取文本长度
            var char_c = ""; //当前字符
            var char_i = 0; //字符索引
...
function textdata_read_line(t,l)
{
    if(t !== -1) //如果资源不为空
    {
        ...
        if(res_type === 2) //如果资源是文本
        {
            var text_len = CT_ResourceArgList[ldata_i+2]; //文本数据长度
